<script>
</script>

<main class="flex flex-col flex-1 p-4">
	<section id="introPage" class="grid grid-cols-1 lg:grid-cols2 gap-8 py-8 sm:py-14">
		<div class="flex flex-col lg:justify-center text-center lg:text-left gap-6 md:gap-8 lg:gap-10">
			<h2 class="font-semibold text-4xl sm:text-5xö md:text-6xl lg:text-7xl">
				Hi! This is <span class="poppins text-violet-400">Dominik</span> Garstenauer
			</h2>
			<h3 class="font-semibold text-3xl sm:text-4xl md:text-5xl lg:text-6xl">
				B.Sc. <span class="poppins text-violet-400">Computer Science </span>
			</h3>
			<p class=" z-index-class text-base sm:text-lg md:text-xl lg:text-2xl">
				I have obtained my degree at the <span class="poppins text-violet-400">
					<a href="https://www.tum.de" target="_blank">
						Techincal University of Munich (TUM)</a
					></span
				>
				in 2023
				<br /> and have studied a <span class="poppins text-violet-400"> wide range </span> of super
				interesting topics like
				<br /> <span class="poppins text-violet-400"> ML & DL, SE, Robotics, MbSE, </span> and much more
				...
			</p>
			<a
				href="https://www.linkedin.com/in/dominik-garstenauer/"
				target="_blank"
				class="blueShadow mx-auto lg:ar-auto text-base sm:text-lg md:text-xl poppins relative overflow-hidden px-5 py-3 group rounded-full bg-white text-slate-950 cursor-pointer"
			>
				<div
					class="absolute top-0 right-full w-full h-full bg-gradient-to-r from-violet-400 to-violet-500 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-200"
				/>
				<h4 class="relative z-9">Get in touch &rarr;</h4>
			</a>
		</div>
		<div class="relative shadow-2xl grid place-items-center">
			<img
				src={"assets/profile.png"}
				alt="profile pic"
				class="object-cover z-[2] max-h-[70wh]"
			/>
		</div>
	</section>
</main>
